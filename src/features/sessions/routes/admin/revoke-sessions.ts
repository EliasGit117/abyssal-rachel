...
import { auth, hasPermissionForRole, Permission } from '@/lib/auth';
...
export const revokeSessions = sessionsAdminBase
  ...
  .handler(async ({ input, errors, context: { headers, user, session: sessionFromReq } }) => {
    const canDelete = hasPermissionForRole(user?.role, { session: [Permission.Delete] });
    if (!canDelete)
      errors.FORBIDDEN();
