...
import { magicLinkForm, MagicLinkForm, TMagicLinkForm } from '@/routes/auth/magic-link/-components/magic-link-form.tsx';
...
export const MagicLinkCard: FC<IMagicLinkCard> = ({ className, ...props }) => {
  const form = useForm<TMagicLinkForm>({
    resolver: zodResolver(magicLinkForm),
    defaultValues: { email: '' }
  });

  const { mutate: signIn, isPending } = useMutation({
    mutationFn: ({ email }: TMagicLinkForm) => authClient.signIn.magicLink({
      email: email
    }),
    onSuccess: (res) => {
      if (!res.error) {
